...
class ZiroomSpiderSpider(CrawlSpider):
    name = 'ziroom_spider'
    allowed_domains = ['cd.ziroom.com']
    # 还没找到办法全自动爬取，所以抓取各城市信息的时候，将注释取消，且修改setting.py中DEFAULT_REQUEST_HEADERS中的host
    ...
    rules = (
        ...
        # Rule(LinkExtractor(allow=r'((https|http|ftp|rtsp|mms)?:\/\/)[^\s]'), callback='parse_page', follow=True),
        Rule(LinkExtractor(allow=r'^((https|http|ftp|rtsp|mms)?:\/\/)[^\s]+'), callback='parse_page', follow=True),
    )
    ...
    def parse_page(self, response):
        ...
        for ziroom in Z_list_box:
            ...
            picture = ""
            try:
                prices_url = ziroom.xpath("./div[2]/div[2]/span[2]/@style").get()
                print(prices_url)
            ...
            img_px_list = ['-0px', '-21.4px', '-42.8px', '-64.2px', '-85.6px', '-107px', '-128.4px', '-149.8px',
                           '-171.2px', '-192.6px']
            try:
                # num_1 = ziroom.xpath(".//div[@class='price']/span[2]/@style").get()
                num_1 = ziroom.xpath("./div[2]/div[2]/span[2]/@style").get()
            ...
            try:
                ...
                num_2 = ziroom.xpath("./div[2]/div[2]/span[3]/@style").get()
            ...
            try:
                ...
                num_3 = ziroom.xpath("./div[2]/div[2]/span[4]/@style").get()
            ...
            try:
                ...
                num_4 = ziroom.xpath("./div[2]/div[2]/span[5]/@style").get()
